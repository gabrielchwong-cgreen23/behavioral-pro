...
test('session feature SQL includes dedupe, fallback-safe grouping, and objective counts', () => {
  ...
  assert.match(cte, /deduped_events_raw AS \(/)
  assert.match(cte, /max\(event_ts\) AS latest_event_ts/)
  assert.match(cte, /latest_event_ts AS event_ts/)
  assert.doesNotMatch(cte, /max\(event_ts\) AS event_ts/)
  ...
  assert.match(cte, /frame_mouse_velocity_avg/)
  assert.match(cte, /final_stage_before_exit/)
  assert.match(cte, /scroll_100_count/)
  assert.match(sql, /provisional_abandoned_checkout/)
  assert.match(sql, /average_mouse_velocity/)
  assert.match(sql, /peak_friction_time_seconds/)
